...
class TaskListViewController: UIViewController {
    ...
    private func refreshTasks() {
        // 1.
        var tasks = Task.getTasks(forKey: Task.saveKey)
        // 2.
...
extension TaskListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            // 3.
            Task.save(tasks, forKey: Task.saveKey) //had an error here - it was initially just "Task.save(tasks). Error was that I was passing an array of [Task] as an argument when the Task.save(_:) method expects a single Task object. to rectify, i updated it by adding "forKey: Task.saveKey)
            // 4.
